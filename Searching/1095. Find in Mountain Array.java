...
class Solution {
    public int findInMountainArray(int target, MountainArray mountainArr) {
        ...
        int ans = binarySearch(mountainArr, 0, peak, target); // as we have to find min index first search for target in
                                                              // ascending array i.e in left part of array

        if (ans == -1) {
            ans = binarySearch(mountainArr, peak + 1, mountainArr.length() - 1, target); // if target is not found in
                                                                                         // ascending array then look in
                                                                                         // descending array i.e right
                                                                                         // part of array
        }
